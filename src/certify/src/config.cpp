// config.cpp - CConfig::InitConfig + GetValue (reconstructed from XKICK_Certify).
// Parses ../config.ini. Each line is whitespace-stripped; '//'-comments and
// blank lines are skipped. Recognised keys fill the matching field. A line that
// equals this deployment's working-directory LEAF name (e.g. "certify") - or that
// contains its full absolute path, for backward compatibility - acts as a section
// header; the NEXT line in that section provides this server's ServerCode. Company
// selects the table path: 100=Korean, 200=Dubai_Arb, 900/other=Default_Eng.
//
// Portable: std::filesystem for cwd; fopen/fgets are standard. No OS headers.
#include "Main.h"
#include <cstdio>
#include <cstring>
#include <cstdlib>
...
bool CConfig::InitConfig()
{
    ...
    // directory (portable): cwd ".../run/certify" -> "certify". A full absolute
    // path is still accepted too, for backward compatibility with old headers.
    char cwd[260];
    char leaf[100];
    ...
    while (std::getline(fs, raw))
    {
        ...
        else if (strstr(line, "Company"))
        {
            ...
            else if (m_nCompany == 900) snprintf(m_sPath, sizeof(m_sPath), "Default_Eng");
        ...
        else if (strstr(line, "Version"))   { GetValue(val, line); m_nVersion = atoi(val); }
